...
description: Reads and reorders the mp inbox, one ranked list of every piece in flight across every project. Use when asked what to work on next, what is in progress or blocked, what is waiting on review, or to reprioritise, annotate or snooze work. Also the bridge for syncing mp state into an external task system. For the piece/PR/stack workflow inside one repo, use managing-monkeypuzzle instead.
---

# mp inbox

The inbox is one ranked list of every piece across every registered project. It
...
| `id` | The piece's durable identifier. It survives renames; `key` does not |
...
| `snoozed` | Snooze already evaluated against now, so no timestamp maths needed |
...
`urgency` is derived on each read rather than stored. `blocked` means an agent
is waiting on a human, `review` means an agent finished or a non-draft PR is
open, `merged` means the PR landed and the piece is ready to clean up.
...
they report where they would go.
...
`mp wait --timeout 5m` blocks until no agent is working. Everything else here
spans projects; `mp wait` works per repo. It fails outside a git repo and only
...
**Key on `id`, not `key`.** `project/piece` is what a human types, and it
changes when either name changes. `id` is minted once and holds for the life of
the piece.
...
- **Never reorder or snooze without being asked.** Rank is the human's own
  judgement about their work, and rewriting it silently destroys the one thing
  the inbox is for. Report the order, suggest a change, let them decide.
- A snooze hides a row from `next`/`prev` until it expires. Say so when you
  set one, and say for how long.
- An empty inbox means no pieces, which is also what an unregistered project
  looks like. Check `mp project list` before you call it no work.
- mp consults the forge for PR state and caches the result briefly. A failure
  there warns and leaves `pr` empty rather than failing the list, so an absent
  `pr` means "unknown", not "no PR".
