...
The target resolves like [`mp switch`](/docs/commands/#mp-switch), except that
...
[command reference](/docs/commands/#shell-completion).
...
sidebar. So the [herdr plugin](https://github.com/jewell-lgtm/monkeypuzzle/blob/main/apps/herdr/README.md) only adds the verbs
...
(see [Input modes](/docs/commands/#input-modes)). Non-interactive calls fail
...
Reference: [Agent commands](/docs/commands/#agent-commands).
...
on the box. See [Remote development](/docs/remote-development/).
...
The CLI doesn't need it. To run your own, see [Self-hosting](/docs/self-hosting/).
