...
`--project <name>` routes any command to a registered project — proxied over
ssh when the entry has a host, run from its path when local. Same command
either way:

```bash
mp create --project api --prompt "add rate limiting"
echo '{"project":"api","prompt":"add rate limiting"}' | mp create --project api
mp list --project api
...
based on your cwd alone, and `--dir` paths must be absolute (they end up inside
single quotes on the remote shell, so `~` does not expand).
...
- The dashboard lists remote projects but doesn't enumerate their pieces;
  `mp list --project <name>` proxies in for the live tree.
