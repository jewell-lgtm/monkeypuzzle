...
# that matches nothing (or ctrl-o) creates the piece instead, and ctrl-d /
# ctrl-x end a piece's life through finish.sh.
#
# `open.sh rows` prints the fzf rows and exits; that is the reload command the
# lifecycle keys use (a shell function can't be an fzf reload target).
...
#   <display>\t<project>\t<piece>\t<worktree>\t<project-path>
...
# switch call, <worktree> feeds the preview, and <project-path> is the repo
# the lifecycle keys run mp in. A piece checked out on a branch that differs
# from its name shows the branch in its label.
...
build_rows() {
	jq -r '
		...
		| .[]
		| [ ($proj + "/" + .label), $proj, .piece, .worktree, $path ]
		| @tsv
	'
}
...
main() {
	...
	local go_json rows projects reload out rc=0 query key selection proj piece
	go_json="$("$(mp_bin)" go --json)"
	rows="$(build_rows <<<"$go_json")"
	projects="$(build_project_rows <<<"$go_json")"
	[[ -n "$rows" ]] || die "no pieces or projects found"

	reload="bash \"$DIR/open.sh\" rows"
	out="$(printf '%s\n' "$rows" | fzf_pick_or_create \
		--with-nth=1 \
		--prompt='piece> ' \
		--header='enter open · ^O create what you typed · ^D done · ^X abandon' \
		--bind "ctrl-d:execute(bash \"$DIR/finish.sh\" done {2} {3} {5})+reload-sync($reload)" \
		--bind "ctrl-x:execute(bash \"$DIR/finish.sh\" abandon {2} {3} {5})+reload-sync($reload)" \
	...
	# wiring is testable.
	case "$key" in
		ctrl-d) exec bash "$DIR/finish.sh" "done" "$(cut -f2 <<<"$selection")" "$(cut -f3 <<<"$selection")" "$(cut -f5 <<<"$selection")" ;;
		ctrl-x) exec bash "$DIR/finish.sh" abandon "$(cut -f2 <<<"$selection")" "$(cut -f3 <<<"$selection")" "$(cut -f5 <<<"$selection")" ;;
	esac
